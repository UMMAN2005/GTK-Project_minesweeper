...
void activate(GtkApplication* app, gpointer userData) {
    ...
    printf("Board length: %d, Board width: %d, Bomb/Flag count: %d\n", N, M, B);
    ...
    // Create a label
    GtkWidget* label = gtk_label_new("Your Label Text");
    gtk_widget_set_hexpand(label, TRUE);
    gtk_widget_set_halign(label, GTK_ALIGN_CENTER);

    // Add the label to the top rows in the grid
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < M; j++) {
            ...
            g_signal_connect(buttons[i][j], "button-press-event", G_CALLBACK(buttonRightClicked), GINT_TO_POINTER(i * M + j));
            gtk_grid_attach(GTK_GRID(grid), buttons[i][j], i, j + 3, 1, 1);  // Adjusted row position
        }
    }
...
void* playAudioThread(void* data) {
    ...
    // Use the system function to execute the command asynchronously
    int result = system(command);

    if (result == 0) {
        printf("MP3 file played successfully.\n");
    } else {
        fprintf(stderr, "Error playing MP3 file.\n");
    }
...
void buttonRightClicked(GtkWidget* widget, GdkEventButton* event, gpointer userData) {
    ...
    if (event->type == GDK_BUTTON_PRESS && event->button == GDK_BUTTON_SECONDARY) {
        ...
        // Check if the button already has an image (flag), remove it
        if (gtk_button_get_image(GTK_BUTTON(widget)) != NULL) {
            gtk_button_set_image(GTK_BUTTON(widget), NULL);
            F++;  // Increase the flag count
        } else {
            // Place a flag image on the button
            GtkWidget* flagImage = gtk_image_new_from_file(
                "C:\\Users\\user\\Desktop\\C-C++programming\\C_programming\\GTK+\\minesweeper\\unnamed.png"
                );
            gtk_button_set_image(GTK_BUTTON(widget), flagImage);
            F--;  // Decrease the flag count
        }

        // Update the flag count label or perform other actions as needed
        // For example: updateFlagCountLabel();
    }
}
